...
description: >
  ...
  с открытым исходным кодом. Здесь вы можете найти классический
  способ внести свой вклад почти в любой проект. А так же узнать
  несколько советов и хаков.
tags:
  - open-source
extra:
  custom_props:
...
же узнать несколько советов и хаков. Мы рассмотрим полнцый цикл
от пробемы до решения, постарясь на каждом этапе дать
дополнительные рекомендации.

Итак, с чего же начинаенается любой вклад. В идеале, с проблемы.
Проблемой можно называть что угодно, в том числе нехватка
каких-то новых фич, отстутствие либо неполнота страниц
...
пробему, сделать несклько попыток по поиску подобных issue, чтобы
не нагржуать лишней работой мейнтейнеров репозитория. Давайте
...
> оффициальной утилиты `gh`. Она позволяет из консоли выполнять
> все базовые операции с GitHub. Я рекомендую использовать её для
> простых операций, для ускорения работы.

Параметр `search` позволяет задавать прям в себе доавлять
...
в нём решение или ответ. Если же нет, то не стейсняйтесь
...
принятs в этом проекте. Создать issue можно командой:
...
в котором настроено все окружение, и проблемма гарантированно
воспроизводима. Либо хотя бы предоставить лог. Профессиональным
подходом так же будет локализазция проблемы, настолько насклько
...
a = fruits.Apple()
korb = fruits.MakeKorb()
korb.put(a)
logistic.send(korb)
```

Конечно, можно было бы так в issue и написать. Но еще лучше будет
самостоятельно поэксперементировать с этим кодом и понять, что
...
коризну, что будет означать баг в методе `put`. Либо же все-таки
...
отправять pull request-ы, ведь вы, скорее всего, не имеете прав
...
гораздно удобнее просто на него сослаться, через номер issue:
...
в какое-то время мы на master ветке выпускаем релиз. В итогде
...
Когда изменения внесены и закомичены, то можно отправлять pull
...
упрощяют и ускоряют процессс ревью. Вы сразу можете увидеть, что
...
пуша они автоматически отобразяться в PR и проверки будут
перезапущенны. Мейнтейнер репозитория даже не начнет ревью вашего
...
Так же старейтесь сделать аккуратную историю коммитов в pull
request-е, чтобы упросить процесс review. В этом вам помогут
[squash](https://www.geeksforgeeks.org/git/git-squash/) коммиты.

Когда вы проравлись через все авто-провкри, не стейсняйтесь
обращаться к архитектору репозитория и просить его ревью. Он либо
сам проревьюит ваш PR, либо делегирует это кому-то из команды. На
GitHub это делается просто через тег: `@username`. Часто
уведомления о том, что пришёл новый pull request могут быть
...
Если вы понимаете, что ваш pull reqeuest полностью закрывает тот
...
полноправным контрьбюьтором в этот проект. Так держать!

Обратите так же внимание на интересную практику, которая, на мой
взгляд, очень професссиональна. Представим что вы обнаружили
и зарепортили какой-то баг в issue. Это уже вклад в репозиторий.
Дальше предлагается создать pull request, в котором вы пишете
тест, который воспроизводит этот баг. Тест конечно-же будет
падать. Но вы присылать pull reqeust, внося тем самым этот тест
...
вы добавили содержаительный тест, а этом всегда очень полезно.
...
массу артефактовв в репозитории: issue, усилиная система тестов,
и пофикшенный баг. Это будет отличным примером реализации TDD
(Test Driven Development) подхода.

Вы можете проверить свои силы уже сейчас, найдя подходящий issue
с помощю этих прекрасных сервсисов:
...
- [Awesome for
  ...
  — Подборка проектов для начинаюх.
...
Не забывайте, что вы и сами можете регестрировать свои проеты на
